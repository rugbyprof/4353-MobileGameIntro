var destroyer = {
	...
	create: function () {
		...
		this.earth = game.add.sprite(0, 0, 'google_earth');
		this.earth.scale.setTo(0.6, 0.6);

		this.earth.animations.add('spin');
		this.earth.animations.play('spin', 1, true);
	...
	update: function () {
		...
		// Check for overlap between game ship and other ships
		
		for(i=0;i<game.multi.count;i++){
			if(game.multi.others.hasOwnProperty(i)){
				game.physics.arcade.overlap(this.player.ship, game.multi.others[i].ship, this.bumpShip, null, this);
			}
		}
	...
	bumpShip: function(ship1, ship2) {
		if(ship1.y < ship2.y){
			console.log("ship1 higher")
			this.player.flipShip('right');
			if(ship1.angle == 0){
				if(ship1.x < ship2.x){
					ship2.x += 50;
					ship2.y += 50;
